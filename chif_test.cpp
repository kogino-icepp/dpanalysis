...
void chif_test(){
    ...
    graph -> Draw("AL");
    test_func -> Draw("same");
    /*TF1* f_test = new TF1("f_test","chiF_ndf(x,[0],[1])",0,10);
    
    vector<int> fpara = {1,2,3,4,27};
    vector<Color_t> color = {kBlack,kBlue,kGreen,kRed,kMagenta};
    rep(i,5){
        f_test -> SetParameter(0,6958);
        f_test -> SetParameter(1,fpara[i]);
        TGraph* graph = new TGraph(f_test);
        graph -> SetLineColor(color[i]);
        if(i==0){
            axrange ax = {0,10,0,3000};
            st.Graph(graph,ax);
            graph -> SetTitle("chi_square;x;chi2");
            graph -> Draw("AL");
        }
        else graph -> Draw("L");
    }
    cout << TMath::Exp(TMath::LnGamma(0.5)) << endl;*/
}